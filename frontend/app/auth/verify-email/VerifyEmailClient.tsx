...
} from "../../../src/lib/auth/auth.queries";
...
const Alert = ({
  ...
  return (
    <div className={`${bg} border rounded-lg p-4 mb-4 relative`}>
      <div className="flex items-start">
        <span className="text-lg mr-3">
          {type === "success" ? "✅" : type === "error" ? "❌" : "ℹ️"}
        </span>
        <p className={`${text} flex-1 font-medium`}>{message}</p>
        <button onClick={onClose} className={`${text} text-xl ml-2`}>
          ×
        </button>
      </div>
    </div>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="bg-white p-8 rounded-lg shadow-md max-w-md text-center">
          ...
          <p className="text-gray-600 mb-4">
            {verifyErrorDetails?.response?.data?.message ||
